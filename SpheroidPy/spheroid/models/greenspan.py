...
class ModelParameters:
    ...
    # Derived parameters with default values
    omega1: float = 20.0  # Minimum nutrient concentration
    omega2: float = 100.0  # Maximum nutrient concentration
    A: float = 12.0  # Nutrient consumption rate
    k: float = 1000.0  # Nutrient diffusion coefficient
    beta1: float = 300.0  # Critical waste concentration
    kappa: float = 500.0  # Waste diffusion coefficient

    def __post_init__(self):
        """Calculate P parameter after initialization"""
        self.P = (
                self.A
                / (self.k * (self.omega2 - self.omega1))
                * self.beta1
                * self.kappa
                / (self.Q ** 2)
        )
    ...
    @classmethod
    def as_list(cls):
        """Return parameter names as a list for curve fitting."""
...
class GreenspanModel:
    ...
    def __init__(self, Q: float = 0.7, gamma: float = 3, s: float = 0.4, R1: float = 220.0, R0: float = 180.0,
                 omega1: float = 20.0, omega2: float = 100.0, A: float = 12.0, k: float = 1000000.0,
                 beta1: float = 2300.0, kappa: float = 1000.0):
        """
        ...
        Args:
            ...
            omega1: Minimum nutrient concentration
            omega2: Maximum nutrient concentration
            A: Nutrient consumption rate
            k: Nutrient diffusion coefficient
            beta1: Critical waste concentration
            kappa: Waste diffusion coefficient
        """
        self.params = ModelParameters(
            Q=Q, gamma=gamma, s=s, R1=R1, R0=R0,
            omega1=omega1, omega2=omega2, A=A, k=k,
            beta1=beta1, kappa=kappa
        )

        # Drug treatment parameters (kept separate as they're for a different model extension)
        self.drug_params = {
    ...
    def __call__(self, t_array: np.ndarray, Q: Optional[float] = None, gamma: Optional[float] = None,
                 s: Optional[float] = None, R1: Optional[float] = None, R0: Optional[float] = None,
                 omega1: Optional[float] = None, omega2: Optional[float] = None,
                 A: Optional[float] = None, k: Optional[float] = None,
                 beta1: Optional[float] = None, kappa: Optional[float] = None) -> np.ndarray:
        """
        ...
        Args:
            ...
            omega1: Optional override for minimum nutrient concentration
            omega2: Optional override for maximum nutrient concentration
            A: Optional override for nutrient consumption rate
            k: Optional override for nutrient diffusion coefficient
            beta1: Optional override for critical waste concentration
            kappa: Optional override for waste diffusion coefficient
        ...
        # Update parameters if any are provided
        if any(param is not None for param in [Q, gamma, s, R1, R0, omega1, omega2, A, k, beta1, kappa]):
            self.params = ModelParameters(
                ...
                omega1=omega1 if omega1 is not None else original_params.omega1,
                omega2=omega2 if omega2 is not None else original_params.omega2,
                A=A if A is not None else original_params.A,
                k=k if k is not None else original_params.k,
                beta1=beta1 if beta1 is not None else original_params.beta1,
                kappa=kappa if kappa is not None else original_params.kappa
            )

        try:
            # Simulate and concatenate results
            Ro, Ri, Rn = self.simulate(t_array)
            result = np.concatenate([Rn, Ro])
        finally:
    ...
    def plot_evolution(self, t_array: np.ndarray, save_path: Optional[str] = None) -> Tuple[
    ...
    def plot_until(self, t_end: float, num_points: int = 100) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        ax.plot(t_array, Ro_array, color=COLORS['proliferation'], label='Outer (Proliferating)', linewidth=2)
        #ax.plot(t_array, Ri_array, color=COLORS['inhibited'], label='Inhibited', linewidth=2)
        ax.plot(t_array, Rn_array, color=COLORS['necrotic'], label='Necrotic', linewidth=2)
        ax.set_xlabel('Time (hours)')
        ax.set_ylabel('Radius (µm)')
        ax.set_title('Greenspan Model Prediction')
        ax.legend()
        ax.grid(True)
        # plt.show()

        return fig, ax  # Ro_array, Ri_array, Rn_array

    def omega_fun(self, r, Ro, Ri, Rn):  # oxygen and nutrients (as a function of r)
        if r > Ro:
            return self.params.omega2
        elif Rn <= r <= Ro:
            return (self.params.omega2 - self.params.A / (6 * self.params.k) * (Ro ** 2 - r ** 2) +
                    self.params.A * Rn ** 3 / (3 * self.params.k) * (1 / r - 1 / Ro))
        else:
            return self.params.omega1

    def beta_fun(self, r, Ro, Ri, Rn):  # waste products (as a function of r)
        if Rn <= r <= Ro:
            return self.params.P / (6 * self.params.kappa) * (Ro ** 2 - r ** 2 - 2 * Rn ** 3 * (1 / r - 1 / Ro))
        elif r < Rn:

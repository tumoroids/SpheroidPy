...
class SpheroidCollection:
    ...
    def growth_rate(self, metric_name: str = 'radius', time_period: str | None = None, 
                    model: str = 'linear', mean: bool = False) -> pd.DataFrame | tuple[float, float]:
        """
        Calculate growth rates for all spheroids in this collection.
        
        Args:
            metric_name: Metric to calculate growth rate for ('radius', 'area', 'fluorescence_*')
            ...
            mean: If True, returns mean ± std across spheroids. If False, returns individual growth rates.
            
        Returns:
            If mean=False: DataFrame with one row per spheroid series, columns: 'growth_rate', 'std_error'
            If mean=True: Tuple (mean_growth_rate, std_error_of_mean) across all spheroids
        """
        growth_rates = []
        std_errors = []
        series_names = []
        ...
        for spheroid_series in self.get_spheroids():
            try:
                rate, std_err = spheroid_series.growth_rate(
                ...
                series_names.append(spheroid_series.name)
            except (ValueError, KeyError) as e:
                # Skip series that fail (e.g., insufficient data)
                logger.warning(f"Could not calculate growth rate for {spheroid_series.name}: {e}")
                continue
        
        if not growth_rates:
            if mean:
                return (np.nan, np.nan)
            return pd.DataFrame(columns=['growth_rate', 'std_error'])
        
        if mean:
            # Return mean ± std error of mean
            mean_rate = np.mean(growth_rates)
            # Standard error of mean = std / sqrt(n)
            sem = np.std(growth_rates, ddof=1) / np.sqrt(len(growth_rates)) if len(growth_rates) > 1 else 0.0
            return (float(mean_rate), float(sem))
        else:
            # Return DataFrame with individual growth rates
            return pd.DataFrame({
